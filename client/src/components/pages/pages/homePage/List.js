...
class List extends Component {
  ...
  onFind = (e) => {
    let danhsach = this.state.products
    let ten = this.state.find.trim().toUpperCase()
    let temp = false
    temp === "" ? temp=false : temp=true
    if (temp === false) {
      return danhsach
    } else {
      danhsach = danhsach.filter(e => { // eslint-disable-line
        if (e.nameProduct.toUpperCase().indexOf(ten) !== -1) {
          return e
        }
      })
    }
  ...
  render() {
    ...
    let products = this.state.danhsach.map((product) => { // eslint-disable-line
      // eslint-disable-line
      if (this.state.idHandleList === "") {
        return (
          <li
            ...
            title="Xem chi tiết"
            key={product._id}
            onClick={() => this.onDetail(product._id)}
          ...
          >
            <div className="card ">
              ...
              <div className="card__text">
                ...
                <h6 className="text__priceup">
                  <span className="text__price-sale">
                    {this.format2(
                      product.priceProduct -
                        (product.priceProduct * product.saleProduct) / 100
                    )}
                  </span>
                </h6>
                <h6 className="text__pricedown">
                  <span className="text__vnd"></span>
                  <span className="text__price-cost">
                    {this.format2(product.priceProduct)}
                  </span>
                  <span className="text__percent">
                    {" "}
                    -{product.saleProduct}%
                  </span>
                </h6>
    ...
    let listProduct = this.state.listArray.map((e) => {
      return (
        <li
          key={e._id}
          onClick={() => this.onHandleList(e._id, e.nameProduct)}
          className="list__li main__span"
        >
          {e.nameProduct}
        </li>
      );
      ...
      <div className="container">
        <div className="main list">
          <aside className="list__aside">
            ...
            <ul>
              <li
                onClick={() => this.onHandleList("", "Tất cả")}
                className="list__li main__span"
              >
                Tất cả
              </li>
              ...
              <button
                ...
                id="showList"
                className="btn text-center main__span"
              >
                Xem thêm
              </button>
            </ul>
          ...
          <article className="list__article">
            ...
            <hr style={{ width: "80%", margin: "10px auto" }} />
            <div className="list__fill">
              <input onChange={this.find} name="find" type="text" className="list__filter" placeholder="Tìm kiếm" />
              <button onClick={this.onFind} className="main__btn2"><i className="list__icon fas fa-search"></i></button>
            </div>
